...
def assert_url_not_found_exception(response: Response):
    assert response.status_code == URLNotFoundError.status_code
    response_json = response.json()
    assert response_json.get("msg") == URLNotFoundError.message
    assert response_json.get("code") == URLNotFoundError.error_code


def assert_url_restricted_exception(response: Response):
    assert response.status_code == URLRestricted.status_code
    response_json = response.json()
    assert response_json.get("msg") == URLRestricted.message
    assert response_json.get("code") == URLRestricted.error_code
...
async def test__shorten_url(async_http_client, urls):
    for url in urls:
        ...
        assert response.status_code in [status.HTTP_200_OK, URLRestricted.status_code]
        response_json = response.json()
        if response.status_code == status.HTTP_200_OK:
            ...
            assert_url_restricted_exception(response)
...
async def test__deactivate_link__unknown_url(async_http_client):
    ...
    assert_url_not_found_exception(response)
